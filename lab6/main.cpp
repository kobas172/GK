...
//------------------------------------------------------------------------------------------------------------------------------
GLfloat verify[] = { //wspolrzedne wierzcholkow sciany
	5.0f,  0.0f, -5.0f,
	5.0f,  2.0f, 0.0f,
	-3.0f,  2.0f,  -2.0f,
	-3.0f,  0.0f,  3.0f,
...
float pixels_wall1[] = { //tekstura o wymiarach 2x2; dla kazdego punktu okreslone skladowe RGB koloru 
	1.0f, 1.0f, 1.0f,   0.1f, 0.1f, 0.1f,	0.5f, 0.5f, 0.5f,
	1.0f, 1.0f, 1.0f,   0.1f, 0.1f, 0.1f,	0.5f, 0.5f, 0.5f,
	1.0f, 1.0f, 1.0f,   0.1f, 0.1f, 0.1f,	0.5f, 0.5f, 0.5f,
};
	...
	while (true)
	{
		if (SDL_PollEvent(&windowEvent))
		{
			...
			if (windowEvent.type == SDL_KEYDOWN)
			{
				...
				case SDLK_UP:
					temp = position + direction * glm::vec3(0.2f, 0.0f, 0.2f);
					rusz = true;
					for (int i = 0; i < 3; i++)
					{
						int counter = 0;
						double x1 = verify[i * 6];
						double z1 = verify[i * 6 + 2];

						double x2 = verify[i * 6 + 3];
						double z2 = verify[i * 6 + 5];
						...
						if (temp[0] >= x2 - 1.0f && temp[0] <= x1 + 1.0f)
							counter++;
						if (temp[2] >= z2 - 1.0f && temp[2] <= z1 + 1.0f)
							counter++;
					...
					if (rusz)
						position += direction * glm::vec3(0.2f, 0.0f, 0.2f);
					break;
				case SDLK_DOWN:
					temp = position - direction * glm::vec3(0.2f, 0.0f, 0.2f);
					rusz = true;
				...
				case SDLK_LEFT:
					...
					direction = glm::vec3(cos(fi), 0.0f, sin(fi));
					break;
				case SDLK_RIGHT:
					fi = fi + 0.01;
					direction = glm::vec3(cos(fi), 0.0f, sin(fi));
