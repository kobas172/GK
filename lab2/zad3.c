...
int red, green, blue, yellow, black, white, grey;
...
void drawS(Display* display, Window window, GC gc, int dx, int dy, float scaleX, float scaleY, int colorLitery, int colorObwodki)
{
	int S = 4, S1 = 5;

	XPoint literkaS[] = {{10, 0}, {20, 0}, {30, 15}, {0, 15}};

	XPoint obwodkaS1[] = {{10, 0}, {20, 0}, {30, 15}, {0, 15}, {10, 0}};
	...
	for(int i=0; i<S1; i++){ 
		obwodkaS1[i].x *= scaleX;
		obwodkaS1[i].y *= scaleY;
        	obwodkaS1[i].x += dx;
        	obwodkaS1[i].y += dy; 	  
    	}
    	...
    	XSetLineAttributes(display, gc, 3, LineSolid, 1, 1);
 	XSetForeground(display, gc, colorLitery);
   	XFillPolygon(display, window, gc, literkaS, S, Complex, CoordModeOrigin);
   	XSetForeground(display, gc, colorObwodki);
   	XDrawLines(display, window, gc, obwodkaS1, S1, LineSolid);
}
  ...
  while (to_end == FALSE)
  {
    ...
    switch(event.type)
    {
      case Expose:
        ...
        {
		drawS(display, window, gc, 150, 0, 25, 40, grey, black);
    		drawP(display, window, gc, 350, 150, 10, 10, green, black);
    		drawG(display, window, gc, 300, 150, 10, 10, yellow, black);
        }
        break;
      ...
      case MotionNotify:
        ...
        drawP(display, window, gc, event.xmotion.x+50-100, event.xmotion.y-100, 10, 10, green, black);
    	drawG(display, window, gc, event.xmotion.x-100, event.xmotion.y-100, 10, 10, yellow, black);
